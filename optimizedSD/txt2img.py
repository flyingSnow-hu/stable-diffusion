...
def main():
    ...
    parser.add_argument(
        "--runtime_cfg",
        type=str,
        help="path to runtime configs",
        default="runtime.yaml",
    )
    opt = parser.parse_args()
    runtime_cfg = OmegaConf.load(f"{opt.runtime_cfg}")
    ...
    sd = load_model_from_config(f"{runtime_cfg.ckpt.t2i}")
    li, lo = [], []
    ...
    model.cdevice = runtime_cfg.device
    ...
    if device != "cpu" and opt.precision == "autocast":
        model.half()
        modelCS.half()
